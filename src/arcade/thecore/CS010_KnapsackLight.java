...
public class CS010_KnapsackLight {
    ...
    static int solution(int value1, int weight1, int value2, int weight2, int maxW) {
        ...
        treeMap.put(value1, weight1);
        treeMap.put(value2, weight2);

        if (weight1 + weight2 <= maxW) return value1 + value2;
        else if (treeMap.lastEntry().getValue() <= maxW) return treeMap.lastKey();
        else if (treeMap.firstEntry().getValue() <= maxW) return treeMap.firstKey();
        return 0;
    }
}
