...
to write a function to determine what the last page of the book is that you can number given the current page
and numberOfDigits left. A page is considered numbered if it has the full number printed on it (e.g. if we are
working with page 102 but have ink only for two digits then this page will not be considered numbered).
 ...
 */
public class CS046_PagesNumberingWithInk {
    ...
    static int solution(int current, int numberOfDigits) {
        ...
        if (numberOfDigits < length) return 0;
        while (length <= numberOfDigits) {
            lastPage++;
            numberOfDigits -= length;
            length = String.valueOf(lastPage).length();
        }
        return lastPage - 1;
    }
}
