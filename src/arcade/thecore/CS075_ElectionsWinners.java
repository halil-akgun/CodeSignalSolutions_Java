...
an integer k equal to the number of voters who haven't cast their vote yet,
 ...
 */
public class CS075_ElectionsWinners {
    ...
    static int solution(int[] votes, int k) {
        ...
        return k == 0 ? (Arrays.stream(votes).filter(t -> t >= max).toArray().length == 1 ? 1 : 0)
                : Arrays.stream(votes).filter(t -> t + k > max).toArray().length;
    }
}
